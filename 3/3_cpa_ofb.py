...
def cpa_ofb_decryption(encrypted_data, initial_vector, block_size, prime, generator, key):
    ...
    print("After dividing the data into blocks of size: " + str(block_size) + " => ")
    print(split_string)
    ...
    for i in range(list_length):
        ...
        if(len(split_string[i]) == block_size):
            split_decryption = decryption_in_cpa(split_string[i], prf)
            decrypted_data = decrypted_data + split_decryption
            print("The xor res b/w data and prf is: " + str(split_decryption))
        # Cut the prf to the data's length and then xor
        else:
            data_len = len(split_string[i])
            modified_prf = prf[0:data_len]
            split_decryption = decryption_in_cpa(split_string[i], modified_prf)
            decrypted_data = decrypted_data + split_decryption
            print("The xor res b/w data and prf is: " + str(split_decryption))
...
def generate_cpa_ofb(prime, generator, data, block_size, key):
    ...
    for i in range(list_length):
        ...
        ''' Length of the data is equal to block size, straight xor with prf '''
        if(len(split_string[i]) == block_size):
            split_encryption = encryption_in_cpa(split_string[i], prf)
            encrypted_data = encrypted_data + split_encryption
            print("The xor res b/w data and prf is: " + str(split_encryption))
        # Cut the prf to the data's length and then xor
        else:
            data_len = len(split_string[i])
            modified_prf = prf[0:data_len]
            split_encryption = encryption_in_cpa(split_string[i], modified_prf)
            encrypted_data = encrypted_data + split_encryption
            print("The xor res b/w data and prf is: " + str(split_encryption))
        ''' reset the initial vector to prf for next run '''
        initial_vector = prf
...
def main():
    ...
    data = input("Enter the data to encrypt in binary: ")

    block_size = int(input("Enter (block_size = key_size) for the data: "))

    ''' length of the key and seed avaialble in following link '''
    ''' https://www.ccs.neu.edu/home/wichs/class/crypto-fall15/lecture9.pdf '''
    key = input("Enter the key in binary of length " + str(block_size) + ": ")

    print("\n")

    generate_cpa_ofb(prime, generator, data, block_size, key)
